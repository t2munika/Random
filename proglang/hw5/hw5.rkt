...
(define (mupllist->racketlist mlist)
  (cond
    [(aunit? mlist) null]
    [apair? (cons (apair-e1 mlist) (mupllist->racketlist (apair-e2 mlist)))]))
                     ...
                     (if (closure? clo)
                         (if (fun-nameopt fn)
                             (eval-under-env (fun-body fn)
                                             (list* (cons (fun-nameopt fn) clo)
                                                    (cons (fun-formal fn) arg)
                                                    (closure-env clo)))
                             (eval-under-env (fun-body fn)
